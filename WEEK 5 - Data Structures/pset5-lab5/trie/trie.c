...
bool checkWord(node *cursor, char *prefix, char *word);
...
int main(int argc, char *argv[])
{
    ...
    if (checkWord(root, "", get_string("Check word: ")))
    {
        printf("Found!\n");
    }
    else
...
bool checkWord(node *cursor, char *prefix, char *word)
{
    // Verificar si el nodo actual es una palabra completa
    if (cursor->is_word)
    {
        if (strcmp(prefix, word) == 0)
        {
            return true; // Se encontró la palabra buscada
        }
    }
    ...
    for (int i = 0; i < SIZE_OF_ALPHABET; i++)
    {
        ...
        {
            // Agregar la letra correspondiente al prefijo
            char letter = 'a' + i;
            char *newPrefix = malloc(strlen(prefix) + 2);  // Reservar memoria para el nuevo prefijo
            sprintf(newPrefix, "%s%c", prefix, letter);
            // Recursivamente comprobar si se encuentra la palabra desde el nodo hijo
            if (checkWord(cursor->children[i], newPrefix, word))
            {
                free(newPrefix);
                return true; // Se encontró la palabra buscada en el subárbol actual
            }
            // Liberar la memoria del nuevo prefijo
            free(newPrefix);
        }
    }
    return false; // La palabra no se encontró en el subárbol actual
}
